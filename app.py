...
Ghanaian Food Recognition with Diabetic Dietary Recommendations (v3.1)
"""
...
from config import (
    CAPTURES_DIR, EXPORTS_DIR, CONFIDENCE_THRESHOLD,
...
def get_segmenter():
    ...
    if _segmenter is None:
        # Try trained model first, fall back to pretrained YOLOv8s-seg
        for candidate in [
            ...
            if os.path.exists(candidate) or candidate == 'yolov8s-seg.pt':
                _segmenter = FoodSegmenter(candidate)
                break
    return _segmenter
...
if __name__ == '__main__':
    print("\nGhanaian Food Analysis System — Flask SPA v3.1")
    print("=" * 48)
